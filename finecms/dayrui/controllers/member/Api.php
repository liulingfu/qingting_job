...
class Api extends M_Controller {
    public function qtapi_register(){
        $this->load->model('QtUser_model');
        ...
        $qtUser = new QtUser_model();
        if($qtUser->register($data)){
            $res = array(
                "code"=>0,
                "msg" =>"注册成功"
            );
    ...
    public function qtapi_login(){
        ...
        $qtUser = new QtUser_model();
        if($qtUser->login($data,$resultRow)){
            $res = array(
                "code"=>0,
                "msg" =>"登陆成功",
                "userId"=>$resultRow["id"]
    ...
    public function qtapi_shoucang(){
        $this->load->model('QtUser_model');
        $userId = $this->input->get('userId');
        $jobId = $this->input->get('jobId');
        $jobType = $this->input->get('jobType');
        $data = array(
            "userId"=>$userId,
            "jobId"=>$jobId,
            "jobType"=>$jobType
    ...
    public function qtapi_shoucanglist(){
        $this->load->model('QtUser_model');
        $userId = $this->input->get('userId');
        $jobType = $this->input->get('jobType');
        $data = array(
        ...
        $qtUser = new QtUser_model();
        $resarr = $qtUser->shoucanglist($data);
